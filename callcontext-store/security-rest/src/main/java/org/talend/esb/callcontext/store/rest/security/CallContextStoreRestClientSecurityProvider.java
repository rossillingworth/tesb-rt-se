...
import org.talend.esb.callcontext.store.rest.security.AbstractRestSecurityProvider;
import org.talend.esb.callcontext.store.rest.security.STSClientCreator;
import org.talend.esb.callcontext.store.rest.security.STSRESTOutInterceptor;

public abstract class CallContextStoreRestClientSecurityProvider extends AbstractRestSecurityProvider {
    ...
    protected JAXRSClientFactoryBean getClientFactory() {
        if (null == cachedClientFactory) {
            ...
            if (Authentication.BASIC == callContextStoreAuthentication) {
                factoryBean.setUsername(authenticationUser);
                factoryBean.setPassword(authenticationPassword);
            }

            if (Authentication.SAML == callContextStoreAuthentication) {
